...
(function (root) {
  ...
  root._luliyInitPost=function(){
    ...
    var pbody=document.getElementById('postBody');
    document.querySelectorAll('a[href^="http"]').forEach(function(a){if(!a.href.includes('luliy6.github.io')&&!a.href.includes('luliy.indevs.in'))a.target='_blank';});
  ...
  /* 立即恢复主题 & 皮肤（防 FOUC）
   * 注意：脚本在 <head> 中加载时 document.body 可能为 null，
   * 必须做空值保护，否则会抛出 TypeError 导致后续所有功能失效。
   */

...
一、enhance.js 功能对照表
模块/方法 核心功能 
ready DOM加载完成，统一调度所有初始化逻辑 
fetchPosts 异步请求、加载文章数据 
initLocalStorage 初始化本地存储，持久化用户配置、状态 
initProgressBar 页面顶部加载/滚动进度条 
initDynamicTitle 动态切换浏览器标签标题 
initUptime 统计并展示网站运行时长 
initThemeRipple 点击水波纹动效 
initParticles 页面粒子背景特效 
getACtx / playSfx 音频底层能力、播放点击/交互音效 
initSfxEvents 绑定全局音效触发事件 
initClickSparks 鼠标点击火花特效 
initHeroCluster 首页头部图标/图形集群动效 
initHeroBanner 首页头部横幅渲染与交互 
initTagEnhance 文章标签美化、交互增强 
initLightbox 图片点击放大灯箱功能 
initToolbar 全局工具栏及按钮交互 
SINKS 多套配色/外观方案集合 
applySink 切换并应用选中的配色方案 
sha256 SHA256哈希加密算法 
LOCK_HASH / LOCK_KEY 私密文章加密密钥、校验哈希值 
isFavoritesPost / showLock 判断收藏文章、展示加密文章锁层 
initCards / buildCard 文章卡片初始化、DOM渲染 
initCodeBlocks / applyCodeBlocks 代码块高亮、样式美化、附加功能 
initSakura 全屏樱花飘落动画 
initArticleTocSpy 文章目录滚动监听、锚点高亮 
_luliyInitPost 文章详情页专属初始化 
_luliyInitIndex 首页/列表页专属初始化 

二、enhance.css 样式对照表
选择器 作用区域/功能 
#luliy-progress-bar 页面进度条样式 
#luliy-theme-ripple 点击水波纹容器样式 
#luliy-hero-cluster 首页头部图形集群容器 
#luliy-hero-info / #luliy-hero-name / #luliy-avatar-clock 首页头部信息、昵称、头像计时组件 
#luliy-hero-banner 首页横幅整体样式 
#luliy-pinned-section 置顶文章板块样式 
.luliy-card-grid / .luliy-card / .luliy-card-inner 文章卡片网格、卡片外壳、内部容器 
.luliy-card-date / .luliy-card-title / .luliy-card-tags / .luliy-card-pill 卡片内日期、标题、标签、角标 
.mac-btn / .mac-btn-red / .mac-btn-yellow / .mac-btn-green 代码块模拟Mac窗口按钮 
#luliy-lightbox / #luliy-lightbox-close 图片灯箱、关闭按钮 
#luliy-toolbar / .luliy-tb-btn 工具栏、工具栏按钮 
#luliy-sink-menu / .luliy-sink-opt / .luliy-sink-dot 配色方案菜单、选项、选中标识 
#luliy-lock-overlay / .luliy-lock-box 加密文章遮罩、密码弹窗 
#luliy-sakura-canvas 樱花动画画布 
.luliy-toc-injected-hdr / .luliy-toc-injected-totop 文章目录标题、回到顶部按钮 
data-luliy-theme / data-skin / data-color-mode 主题、皮肤、明暗模式标识（样式切换钩子） 

三、config.json 配置项对照表
配置字段 配置说明 
title 网站主标题 
subTitle 网站副标题 
avatarUrl 头像地址 
...
email 联系邮箱 
startSite 网站上线时间 
onePageListNum 单页文章展示数量 
singlePage 开启/关闭单页模式 
iconList 导航图标/菜单配置 
exlink 外部链接配置 
i18n 多语言国际化 
UTC 时区设置 
urlMode 链接地址规则 
themeMode 默认明暗主题模式 
dayTheme / nightTheme 日间、夜间主题配色 
yearColorList 分类/年度配色组 
commentLabelColor 评论标签颜色 
showPostSource 是否显示文章来源 
needComment 是否开启评论功能 
rssSplit RSS订阅内容分割规则 
bottomText 页面底部自定义文案 
ogImage 社交分享预览图 
primerCSS 代码块基础样式 
allHead 全局头部通用注入内容 
script 全局自定义脚本 

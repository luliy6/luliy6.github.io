enhance.js — Luliy Blog v6
   ...
   ④  运行时间
   ⑤  暗色模式波纹
   ⑥  粒子背景（80粒子，彩虹色 hue 240-300，引力/排斥/连线/光晕）
   ⑦  Web Audio 音效
   ⑧  点击粒子火花
   ⑨  头像时钟 & 链接到 /about
   ⑩  标签页增强
   ⑪  图片灯箱
   ⑫  浮动工具栏（主页/GitHub/主题/音效/皮肤/背景）
...
(function (root) {
      ...
      if (data && typeof data === 'object') {
        return Object.keys(data).filter(function(k){return k!=='labelColorDict';}).map(function(k){
          ...
          return {title:p.title||p.name||k, link:p.link||p.url||('post/'+k+'.html'),
            created:p.created||p.date||p.updated||'', labels:p.labels||p.tags||[], desc:p.desc||p.description||''};
        });
      }
      return [];
    }
  ...
  function initLocalStorage() {
    var defs = {'luliy-sfx':'1','luliy-particles':'1','luliy-petals':'1',
                'luliy-theme':'default','luliy-skin':'default','luliy-bg':''};
  ...
  function initUptime() {
    ...
    el.style.cssText = 'text-align:center;font-size:12px;color:#888;padding:8px 0;margin-top:18px';
    document.body.appendChild(el);
    var start = new Date('2026/05/30 00:00:00').getTime();
    function upd() {
      var d = Date.now() - start;
      if (d < 0) { el.innerHTML = '🚀 博客即将上线，敬请期待...'; return; }
      el.innerHTML = '🌱 已运行：' + Math.floor(d/86400000) + '天 ' +
  ...
  function initThemeRipple() {
    function ripple() {
      ...
      el.style.cssText = 'position:fixed;top:'+cy+'px;left:'+cx+'px;width:0;height:0;border-radius:50%;' +
  ...
  function initParticles() {
    ...
    for (var i=0; i<80; i++) {
      pts.push({
        ...
        hue: Math.floor(Math.random()*60)+240   /* 彩虹色 hue 240-300 */
      });
    }
    ...
    function tick() {
      ...
      var alpha = dark ? 0.70 : 0.45;   /* 暗色模式透明度更高 */

      pts.forEach(function(p){
        if (mouse.active) {
          var dx = mouse.x-p.x, dy = mouse.y-p.y, d = Math.sqrt(dx*dx+dy*dy);
          if (d < 220 && d > 60) {                 /* 吸引区 */
            var f = 0.018*(1-d/220); p.vx += dx/d*f; p.vy += dy/d*f;
          } else if (d <= 60) {                    /* 排斥区 */
            p.vx -= dx/d*0.042; p.vy -= dy/d*0.042;
          }
        }
        p.vx *= 0.980; p.vy *= 0.980;
      ...
      for (var a=0; a<pts.length; a++) {
        for (var b=a+1; b<pts.length; b++) {
          ...
          if (dd < 140) {
            ...
            ctx.strokeStyle = 'hsla(260,70%,65%,'+(1-dd/140)*(dark?0.30:0.18)+')';
  ...
  function playSfx(type) {
    ...
    try {
      if (type==='click') {
        ...
        o.type='square'; o.frequency.setValueAtTime(880,ctx.currentTime); o.frequency.exponentialRampToValueAtTime(380,ctx.currentTime+0.06);
        g.gain.setValueAtTime(0.04,ctx.currentTime); g.gain.exponentialRampToValueAtTime(0.0001,ctx.currentTime+0.07);
        o.start(); o.stop(ctx.currentTime+0.07);
      } else if (type==='sci') {
        var o2=ctx.createOscillator(),g2=ctx.createGain(); o2.connect(g2); g2.connect(ctx.destination);
        o2.type='sine'; o2.frequency.setValueAtTime(440,ctx.currentTime); o2.frequency.exponentialRampToValueAtTime(880,ctx.currentTime+0.12); o2.frequency.exponentialRampToValueAtTime(660,ctx.currentTime+0.22);
        ...
        [0,0.08,0.16].forEach(function(delay,idx){
          ...
          gt.gain.setValueAtTime(0.05,ctx.currentTime+delay); gt.gain.exponentialRampToValueAtTime(0.0001,ctx.currentTime+delay+0.18);
  ...
  function initClickSparks() {
    var colors = ['#ff6b9d','#ffcd3c','#6bceff','#a78bfa','#34d399','#fb923c'];
    document.addEventListener('click', function(e){
      for (var i=0; i<14; i++) (function(){
        var s = document.createElement('div');
        var angle = Math.random()*360, dist = Math.random()*55+18;
        s.style.cssText = 'position:fixed;left:'+e.clientX+'px;top:'+e.clientY+'px;width:7px;height:7px;border-radius:50%;pointer-events:none;z-index:99999;' +
          'background:'+colors[Math.floor(Math.random()*colors.length)]+';transform:translate(-50%,-50%);transition:transform 0.62s ease,opacity 0.62s ease;';
  ...
  function initTagEnhance() {
    ...
    function wire() {
      ...
      tb.style.cssText = 'display:flex;align-items:center;gap:10px;margin:0 0 14px;padding:10px 14px;border:1px solid rgba(9,105,218,0.14);border-radius:10px;background:rgba(9,105,218,0.04);';
      tb.innerHTML = '<input id="tag-filter-inp" type="search" placeholder="筛选标签..." autocomplete="off" style="flex:1;min-width:0;height:34px;border:1px solid rgba(9,105,218,0.2);border-radius:7px;padding:0 10px;font-size:13px;"><span id="tag-filter-cnt" style="font-size:13px;color:#57606a;white-space:nowrap;"></span>';
      tl.parentNode.insertBefore(tb, tl);
      var inp = tb.querySelector('#tag-filter-inp'), cnt = tb.querySelector('#tag-filter-cnt');
      function apply() {
  ...
  /* ═══ ⑫ 浮动工具栏 ════════════════════════════════════
   *  按钮（从上到下）：主页 / GitHub / 文章主题 / 阅读皮肤 / 背景图 / 音效
   * ══════════════════════════════════════════════════════ */
  var THEMES = [
  ...
  function initToolbar() {
    ...
    function mkBtn(emoji, tip, tag) {
      var b = document.createElement(tag||'button');
      b.className = 'luliy-tb-btn'; b.setAttribute('data-tip', tip);
      if (tag!=='a') b.type = 'button';
      b.innerHTML = emoji; return b;
    }

    /* 主页 */
    var btnHome = mkBtn('🏠','主页','a'); btnHome.href = '/';
    /* GitHub */
    var btnGH = mkBtn('','GitHub','a');
    btnGH.href = 'https://github.com/luliy6'; btnGH.target = '_blank';
    ...
    /* 文章主题 */
    var thWrap = document.createElement('div'); thWrap.style.cssText='position:relative;';
    var btnTh = mkBtn('🎨','文章主题');
    var thMenu = document.createElement('div'); thMenu.id='luliy-theme-menu';
    THEMES.forEach(function(t){
      ...
      b.addEventListener('click', function(){ applyTheme(t.id); thMenu.classList.remove('is-open'); playSfx('click'); });
      thMenu.appendChild(b);
    });
    btnTh.addEventListener('click', function(e){ e.stopPropagation(); thMenu.classList.toggle('is-open'); skMenu.classList.remove('is-open'); bgMenu.classList.remove('is-open'); });
    thWrap.appendChild(thMenu); thWrap.appendChild(btnTh);

    /* 阅读皮肤 */
    var skWrap = document.createElement('div'); skWrap.style.cssText='position:relative;';
    var btnSk = mkBtn('🌈','阅读皮肤');
    var skMenu = document.createElement('div'); skMenu.id='luliy-skin-menu';
    SKINS.forEach(function(s){
      ...
      b.addEventListener('click', function(){ applySkin(s.id); skMenu.classList.remove('is-open'); playSfx('click'); });
      skMenu.appendChild(b);
    });
    btnSk.addEventListener('click', function(e){ e.stopPropagation(); skMenu.classList.toggle('is-open'); thMenu.classList.remove('is-open'); bgMenu.classList.remove('is-open'); });
    skWrap.appendChild(skMenu); skWrap.appendChild(btnSk);

    /* 背景图 */
    var bgWrap = document.createElement('div'); bgWrap.style.cssText='position:relative;';
    var btnBg = mkBtn('🖼️','背景图');
    var bgMenu = document.createElement('div'); bgMenu.id='luliy-bg-menu';
    bgMenu.innerHTML =
      '<div style="font-size:12px;color:#888;padding:2px 4px 6px;">输入图片链接</div>' +
      '<div id="luliy-bg-input-row">' +
        '<input id="luliy-bg-url" type="url" placeholder="https://..." autocomplete="off">' +
        '<button id="luliy-bg-apply" type="button">应用</button>' +
      '</div>' +
      '<button id="luliy-bg-clear" type="button" style="margin-top:6px;width:100%;">清除背景</button>';
    bgMenu.querySelector('#luliy-bg-apply').addEventListener('click', function(){
      var url = bgMenu.querySelector('#luliy-bg-url').value.trim();
      if (url) { applyBg(url); bgMenu.classList.remove('is-open'); playSfx('click'); }
    });
    bgMenu.querySelector('#luliy-bg-clear').addEventListener('click', function(){
      bgMenu.querySelector('#luliy-bg-url').value=''; applyBg(''); bgMenu.classList.remove('is-open'); playSfx('click');
    });
    btnBg.addEventListener('click', function(e){ e.stopPropagation(); bgMenu.classList.toggle('is-open'); thMenu.classList.remove('is-open'); skMenu.classList.remove('is-open'); });
    ...
    /* 关闭菜单 */
    document.addEventListener('click', function(){ thMenu.classList.remove('is-open'); skMenu.classList.remove('is-open'); bgMenu.classList.remove('is-open'); });

    bar.appendChild(btnHome); bar.appendChild(btnGH); bar.appendChild(thWrap);
    bar.appendChild(skWrap);  bar.appendChild(bgWrap); bar.appendChild(btnSfx);
    document.body.appendChild(bar);
  ...
  function showLock() {
    ...
    pb.style.filter='blur(18px)'; pb.style.userSelect='none'; pb.style.pointerEvents='none';
    ...
    function tryUnlock() {
      ...
      if (sha256(inp.value)===LOCK_HASH) {
        ...
        pb.style.filter=''; pb.style.userSelect=''; pb.style.pointerEvents='';
  ...
  /* ═══ ⑭ 樱花花瓣 ════════════════════════════════════════ */
  function initPetals() {
    if (localStorage.getItem('luliy-petals')==='0') return;
    if (document.getElementById('luliy-petal-canvas')) return;
    var canvas = document.createElement('canvas');
    canvas.id = 'luliy-petal-canvas';
    document.body.insertBefore(canvas, document.body.firstChild);
    var ctx = canvas.getContext('2d'), W, H;
    function resize(){ W=canvas.width=window.innerWidth; H=canvas.height=window.innerHeight; }
    resize(); window.addEventListener('resize', resize, {passive:true});
    ...
    /* 花瓣颜色池：粉色系 */
    var colors = ['#ffb7c5','#ff9db0','#ffd3dc','#fce4ec','#f8bbd0','#ff80ab'];
    var petals = [];
    for (var i=0; i<60; i++) {
      petals.push({
    ...
    function drawPetal(ctx, x, y, r, rot) {
      ...
      ctx.beginPath();
      ctx.moveTo(0, -r);
      ctx.bezierCurveTo(r*0.8, -r*0.8, r*0.8, r*0.8, 0, r);
      ctx.bezierCurveTo(-r*0.8, r*0.8, -r*0.8, -r*0.8, 0, -r);
      ctx.closePath();
      ctx.fill();
      ctx.restore();
    ...
    function tick() {
      ...
      petals.forEach(function(p){
        p.y += p.speed;
        p.x += p.drift + Math.sin(frame*0.012*p.swing + p.swingOff)*0.8;
        p.rot += p.rotSpeed;
  ...
  function initCodeBlocks(pbody) {
    pbody.querySelectorAll('pre').forEach(function(pre){
      ...
      var bR=mkBtn('mac-btn-red','复制代码');
      var bY=mkBtn('mac-btn-yellow','折叠代码');
      var bG=mkBtn('mac-btn-green','全屏阅读');
  ...
  if (localStorage.getItem('luliy-petals') !== '0') {
    if (document.body) initPetals();
    else document.addEventListener('DOMContentLoaded', initPetals);
  }

...
(function (root) {
  ...
  var SINKS = [
    ...
    },
    {
      ...
      dot:   '#ff6b35',
      theme: 'vice-city',
      cardPalette: ['#ff6b35', '#00d4ff', '#ff9966', '#00aadd'],
      desc:  '\u9633\u5149\u6700\u76db\u5904\u7684\u9ed1\u6697\u9762'
    }
  ];
  ...
  function applySink(id) {
    ...
    /* Toggle VI hero on index page */
    if (isIndexPage() && !isArchivePage()) {
      if (s.id === 'vice-city') {
        /* Remove standard banner, inject VI hero */
        var stdBanner = document.getElementById('luliy-hero-banner');
        if (stdBanner) stdBanner.remove();
        setTimeout(initVIHero, 60);
      } else {
        /* Remove VI hero if present, restore standard banner */
        ['luliy-vi-hero', 'luliy-vi-story', 'luliy-vi-divider-el'].forEach(function (rid) {
          var el = document.getElementById(rid);
          if (el) el.remove();
        });
        document.querySelectorAll('.luliy-vi-divider').forEach(function (el) { el.remove(); });
        if (!document.getElementById('luliy-hero-banner')) initHeroBanner();
      }
    }
  }

  /* ---- initVIHero: Cinematic fullscreen hero (vice-city theme, index only) -- */
  function initVIHero() {
    if (!isIndexPage() || isArchivePage()) return;
    if (localStorage.getItem('luliy-sink') !== 'vice-city') return;
    if (document.getElementById('luliy-vi-hero')) return;
    ...
    hero.id = 'luliy-vi-hero';

    /* Shards — decorative polygon fragments inspired by GTA VI logo shard effect */
    var SHARDS = [
      { top: '14%', left:  '7%',  w: 56, h: 135, rot1: 10,  rot2: 18,  color: 'rgba(255,107,53,0.14)',  delay: 500 },
      { top: '18%', right: '6%',  w: 42, h: 105, rot1: -25, rot2: -14, color: 'rgba(0,212,255,0.10)',   delay: 700 },
      { top: '62%', left:  '5%',  w: 32, h:  76, rot1:  5,  rot2: 12,  color: 'rgba(255,153,102,0.10)', delay: 600 },
      { top: '68%', right: '7%',  w: 50, h: 118, rot1: -15, rot2: -8,  color: 'rgba(0,170,221,0.12)',   delay: 900 },
      { top: '38%', left: '13%',  w: 22, h:  55, rot1:  42, rot2: 50,  color: 'rgba(255,107,53,0.09)',  delay: 650 },
      { top: '42%', right:'12%',  w: 28, h:  68, rot1: -38, rot2: -28, color: 'rgba(0,212,255,0.08)',   delay: 800 },
      { top:  '8%', left: '32%',  w: 18, h:  44, rot1:  28, rot2: 36,  color: 'rgba(255,153,102,0.07)', delay: 750 },
      { top: '82%', right:'28%',  w: 24, h:  58, rot1: -20, rot2: -12, color: 'rgba(255,107,53,0.08)',  delay: 850 },
    ];
    SHARDS.forEach(function (sd) {
      var shard = document.createElement('div');
      shard.className = 'luliy-vi-shard';
      var posCSS = 'position:absolute;width:' + sd.w + 'px;height:' + sd.h + 'px;background:' + sd.color + ';';
      if (sd.top)   posCSS += 'top:'   + sd.top   + ';';
      if (sd.left)  posCSS += 'left:'  + sd.left  + ';';
      if (sd.right) posCSS += 'right:' + sd.right + ';';
      posCSS += 'transform:rotate(' + sd.rot1 + 'deg) scale(0.85);opacity:0;';
      posCSS += 'transition:opacity 1.2s ease,transform 1.2s ease;';
      shard.style.cssText = posCSS;
      hero.appendChild(shard);
      setTimeout(function () {
        shard.style.opacity = '1';
        shard.style.transform = 'rotate(' + sd.rot2 + 'deg) scale(1)';
      }, sd.delay);
    });

    /* ── Content ─────────────────────────────────────────────── */
    var hContent = document.createElement('div');
    hContent.id = 'luliy-vi-hero-content';

    var eyebrow = document.createElement('div');
    eyebrow.className = 'luliy-vi-eyebrow';
    eyebrow.textContent = 'Luliy Blog \u00b7 2026';

    var titleEl = document.createElement('div');
    titleEl.className = 'luliy-vi-title';
    titleEl.textContent = 'LULIY';

    var subEl = document.createElement('div');
    subEl.className = 'luliy-vi-sub';
    subEl.textContent = '\u6211\u5c06\u65e0\u9650\u8fdb\u6b65';

    var ctas = document.createElement('div');
    ctas.className = 'luliy-vi-ctas';

    var ctaP = document.createElement('a');
    ctaP.className = 'luliy-vi-cta primary';
    ctaP.href = '#posts';
    ctaP.textContent = '\u6d4f\u89c8\u6587\u7ae0';
    ctaP.addEventListener('click', function (e) {
      e.preventDefault();
      var target = document.querySelector('#luliy-pinned-section, .luliy-card-grid');
      if (target) target.scrollIntoView({ behavior: 'smooth', block: 'start' });
      playSfx('click');
    });

    var ctaS = document.createElement('a');
    ctaS.className = 'luliy-vi-cta secondary';
    ctaS.href = '/about';
    ctaS.textContent = '\u5173\u4e8e\u535a\u4e3b';

    ctas.appendChild(ctaP);
    ctas.appendChild(ctaS);
    hContent.appendChild(eyebrow);
    hContent.appendChild(titleEl);
    hContent.appendChild(subEl);
    hContent.appendChild(ctas);
    hero.appendChild(hContent);

    /* ── Scroll indicator ────────────────────────────────────── */
    var scrollHint = document.createElement('div');
    scrollHint.id = 'luliy-vi-scroll';
    scrollHint.textContent = 'SCROLL';
    hero.appendChild(scrollHint);

    /* Fade scroll hint on scroll */
    window.addEventListener('scroll', function () {
      if (scrollHint) scrollHint.style.opacity = String(Math.max(0, 1 - (window.scrollY || 0) / 180));
    }, { passive: true });

    /* ── Insert hero before content ──────────────────────────── */
    content.parentNode.insertBefore(hero, content);

    /* ── Story section ───────────────────────────────────────── */
    var story = document.createElement('div');
    story.id = 'luliy-vi-story';
    story.innerHTML =
      '<span id="luliy-vi-story-eyebrow">LULIY \u00b7 \u526f\u90fd\u5e02 \u00b7 \u535a\u4e3b\u5bfc\u8a00</span>' +
      '<p id="luliy-vi-story-text">\u6b22\u8fce\u6765\u5230 Luliy \u7684\u4e16\u754c\u3002' +
      '\u8fd9\u91cc\u8bb0\u5f55\u7740\u6210\u957f\u3001\u601d\u8003\u4e0e\u65e0\u9650\u8fdb\u6b65\u7684\u8db3\u8ff9\u3002' +
      '\u65e0\u8bba\u9633\u5149\u70bd\u70c8\u8fd8\u662f\u591c\u8272\u6df1\u6c89\uff0c\u6587\u5b57\u662f\u6700\u771f\u5b9e\u7684\u5750\u6807\u3002</p>';
    content.parentNode.insertBefore(story, content);

    /* ── Divider ─────────────────────────────────────────────── */
    var divider = document.createElement('div');
    divider.className = 'luliy-vi-divider';
    divider.id = 'luliy-vi-divider-el';
    content.parentNode.insertBefore(divider, content);

    /* ── Scroll-triggered story reveal ──────────────────────── */
    if (root.IntersectionObserver) {
      var obs = new root.IntersectionObserver(function (entries) {
  ...
  function initToolbar() {
    ...
    var THEME_PALETTES = {
      ...
      'vice-city':  { day: ['rgba(2,6,20,0.90)',      '#ff6b35', '#e8d5b0'],  night: ['rgba(0,2,12,0.96)',    '#00d4ff', '#b8d4e8'] }
    };
  ...
  root._luliyInitIndex = function () {
    initCards();
    /* Hero banner: skip if vice-city (initVIHero handles it) */
    if (localStorage.getItem('luliy-sink') !== 'vice-city') {
      initHeroBanner();
    } else {
      setTimeout(initVIHero, 80);
    }
    initHomeGallery();
  ...
  (function () {
    ...
    var themePalettes = {
      ...
      'vice-city':  { theme: 'vice-city',   c: ['#ff6b35', '#00d4ff', '#ff9966', '#00aadd'] }
    };
